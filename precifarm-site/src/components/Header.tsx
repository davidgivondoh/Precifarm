...
export function Header() {
  ...
  return (
    ...
    >
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16 sm:h-18">
        <Link href="/">
          <LogoMarkFull />
        </Link>
        ...
        {/* Mobile menu */}
        {menuOpen && (
          <div className="absolute top-16 sm:top-18 left-0 right-0 bg-white border-b border-gray-200 shadow-lg flex flex-col p-4 gap-1 md:hidden">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
